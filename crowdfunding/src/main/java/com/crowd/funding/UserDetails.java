    ...
    public static void displayUsers(Connection connection) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            try (ResultSet resultSet = statement.executeQuery(query)) {
                System.out.printf("%-10s %-20s %-20s %-15s%n","UserID", "Name", "Contact Number", "Contribution Amount");
                while (resultSet.next()) {
                    ...
                    System.out.printf("%-10d %-20s %-20s %-15s%n",userId, name, contactNumber, contributionAmount);
                }
            }
        }
    }
